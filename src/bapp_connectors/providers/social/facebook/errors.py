"""
Facebook Graph API error mapping.
...
def classify_graph_error(payload: dict) -> ConnectorError:
    ...
    code = error.get("code")
    message = error.get("message", "Unknown Facebook Graph API error")

    if code == 190:
        return AuthenticationError(f"Facebook access token invalid or expired: {message}")
    if code in _RATE_LIMIT_CODES:
        return RateLimitError(f"Facebook rate limit exceeded (code {code}): {message}")
    if code == 100:
        return ValidationError(f"Facebook Graph API invalid parameter: {message}")
    return ProviderError(f"Facebook Graph API error (code {code}): {message}")
