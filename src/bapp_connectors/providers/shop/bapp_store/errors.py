...
def map_error(status: int, body: str) -> ConnectorError:
    ...
    # 429 must stay retryable -- sync_task is the one caller that bypasses the core client's own 429 handling.
    if status == 429:
        return RateLimitError(f"Company Store rate limited the request: {status} {detail}", status_code=status)
    if 400 <= status < 500:
