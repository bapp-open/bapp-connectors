"""
OpenAI LLM integration tests — runs against the real OpenAI API.

Requires OPENAI_API_KEY env var (see .env).

    set -a && source .env && set +a
    uv run --extra dev pytest tests/llm/test_openai_integration.py -v -m integration -s
"""
...
from tests.llm.conftest import OPENAI_API_KEY, skip_unless_openai

pytestmark = [pytest.mark.integration, skip_unless_openai]


@pytest.fixture
def adapter():
    return OpenAILLMAdapter(credentials={"api_key": OPENAI_API_KEY})
...
class TestOpenAIListModels:
    ...
    def test_list_models(self, adapter):
        ...
        for m in gpt_models[:10]:
            print(f"    {m.id}")

    def test_gpt_4o_in_models(self, adapter):
        models = adapter.list_models()
...
class TestOpenAICompletion:
    ...
    def test_completion_with_specific_model(self, adapter):
        ...
        response = adapter.complete(messages, model="gpt-4o-mini")
        assert response.content
...
class TestOpenAIToolCalling:
    ...
    def test_tool_call_with_result(self, adapter):
        from bapp_connectors.core.dto import ToolCall
...
class TestOpenAIImageGeneration:
    ...
    def test_generate_image(self, adapter):
        result = adapter.generate_image("A small red circle on a white background", size="1024x1024")
        assert isinstance(result, ImageResult)
        assert result.b64_data or result.url
        print(f"\n  Generated image: b64 length={len(result.b64_data)}, revised={result.revised_prompt!r}")
    ...
    def test_edit_image(self, adapter):
        ...
        try:
            ...
            if "billing" in str(e).lower() or "quota" in str(e).lower():
                pytest.skip("OpenAI billing/quota issue")
            raise
        assert isinstance(result, ImageResult)
        assert result.b64_data or result.url
        print(f"\n  Edited image: b64 length={len(result.b64_data)}")
    ...
    def test_complete_with_image(self, adapter):
        ...
        messages = [
            ChatMessage(
                role=ChatRole.USER,
                content=[
                    {"type": "text", "text": "What color is this single-pixel image? Answer in one word."},
...
class TestOpenAICapabilities:
    ...
    def test_implements_embedding(self, adapter):
        assert isinstance(adapter, EmbeddingCapability)

    def test_implements_transcription(self, adapter):
        assert isinstance(adapter, TranscriptionCapability)

    def test_implements_image_generation(self, adapter):
        assert isinstance(adapter, ImageGenerationCapability)

    def test_supports_all(self, adapter):
        assert adapter.supports(LLMPort)
        assert adapter.supports(EmbeddingCapability)
        assert adapter.supports(TranscriptionCapability)
        assert adapter.supports(ImageGenerationCapability)
