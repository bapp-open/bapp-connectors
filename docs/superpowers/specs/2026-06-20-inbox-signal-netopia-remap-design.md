...
Kwargs:
    ...
    email: bapp_connectors.core.dto.EmailSummary — the fetched message.
...
class InboxPollService:
    ...
    def poll(self, *, since=None, until=None, folder="INBOX", limit=50) -> InboxPollResult:
        ...
        # guard: adapter must support fetch_messages (InboxCapability)
        messages = adapter.fetch_messages(since=since, until=until, folder=folder, limit=limit)
        records = []
        for email in messages:
            action = self._dispatch(email, folder)        # email_received.send_robust(...)
            applied = self._apply(adapter, email, folder, action)
            records.append(InboxPollRecord(email=email, action=action, applied=applied))
        return InboxPollResult(folder=folder, fetched=len(messages), records=records)
```
...
- `InboxPollRecord(email: EmailSummary, action: InboxAction | None, applied: bool, error: str = "")`
...
host project calls InboxPollService.poll(since=..., folder=...)
  ...
  -> for each EmailSummary:
       email_received.send_robust(connection, email, folder, ...)
...
Django-package unit tests using a fake adapter and a registered test receiver:
